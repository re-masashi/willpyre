...
def _match_type(annotation, value):
    if annotation is str:
        if type(value) is str:
            ...
            raise ValidationError(f'Wrong type. Got {type(value)} instead of {annotation}.')
    ...
    if isinstance(annotation, Constr):
        if type(value) is str and value >= annotation.min and value <= annotation.max:
            ...
            raise ValidationError(f'Wrong type.')
...
def schema_repr(schema):
    ...
    for field in schema.__FIELDS__.values():
        if field.annotation is int:
            ...
            annotation = 'dict'
        elif isinstance(field.annotation, type) and hasattr(field.annotation, "__FIELDS__"):
